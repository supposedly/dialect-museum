...
2. Allow users to explore the map interactively. Namely,
    ...
    2. Conversely, allow users to scroll through a list of features and see where they're found on the map.
3. That's not good enough, though. An abstract list of features isn't user-friendly. Instead, allow users to _really_ interact with the map. Namely:
    1. Allow users to read texts that have been recorded for different areas on the map. The literature available on Arabic varieties is really good about recording stories in a usable text format, so the main object here is digitizing those stories.
...
I never got to the point where I made a usable frontend (i.e. the map and everything else). The part I left off at was just after I had finished all of the type-system code that makes the project usable for a "creator" (the point where they can rely on the typechecker to make sure they're not making mistakes while documenting a language/dialect) and had started to implement and troubleshoot the runtime code that generates the actual surface forms using the data the creator describes. Because of that, this codebase actually contains an almost-complete phonological and morphological description of my own dialect, and it also contains the seeds of a Lebanese verb conjugator (in that all the data is there) that I think would be more complete than anything else that's out there right now, but it doesn't have the runtime code that actually uses that data correctly.
