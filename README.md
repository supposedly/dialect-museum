...
First things first: this project is **on hiatus** :) In its current state it's also incomplete in that (1) there's no UI yet and (2)
...
## Update: postmortem
I never got those 4&ndash;6 months due to poor life choices, and unfortunately I no longer understand advanced TypeScript or this project's design. Here was the project's purpose, formatted as a list of goals that starts in broadest terms and then narrows down:

1. Be a Sprachatlas. (A map of the locations of different dialect features. I like Lebanese Arabic, so I was starting with a map of Lebanon.)
2. Allow users to explore the map interactively. Namely,
    1. Allow users to click on an area of the map to view features documented for the dialect of that area.
    2. Conversely, allow users to scroll through a list of features and click on any given one to see where it's found on the map.
    3. Also allow them to view correlations (I think I did an okay job of implementing these three things in my first attempt at a dialect atlas, https://github.com/supposedly/lebmap)
3. That's not good enough, though. An abstract list of features isn't user-friendly. Instead, allow users to _really_ interact with the map. Namely:
    1. Allow users to read texts that have been recorded for different areas on the map. The literature available on Arabic varieties is actually really good about recording stories in text format, so the main object here is digitizing those stories.
    2. Allow users to view how a given text might read if it'd instead been recorded in a different dialect on the map.
4. The implementation of those last two bullet points necessarily opens up several more useful features:
    1. Allow users to view how those texts might read if written in different orthographies. The app would ideally be able to convert any given text into literally anything: Arabic script, Maltese Latin script, Arabizi (would need stochasticity), English-pronunciation-guide type Latin script (e.g. شقفة -> "SHE[T](https://en.wikipedia.org/wiki/T-glottalization)-feh"), arbitrary HTML so you can supplement that pronunciation-guide output with e.g. hover tooltips, random other stuff like code that can be fed into a TTS synth... lots of possibilities!
    2. Allow users to construct a thorough 'accent profile' of theirs by answering survey questions. If they also provide info on where their dialect is from, their responses can then go on the map itself.
    3. Lastly, this gives you the option to use this app without the map to provide an **accurate** variation-aware pronunciation widget for use in e.g. online dictionaries. (If the mention of "arbitrary HTML" above makes sense, this is another outlet for it.)
5. On the flipside, make it easy for a linguist or whoever else to set up a new language or new dialect. Let's call this person a "creator".
    1. Let creators easily describe the variable features of a language. By nature of the project these have to be phonological and morphological variables, not syntactic ones (although that would be really cool to think about how to even start implementing...).
    2. Let them also easily describe the range of variants each one of those features has across the dialects of the language.
    3. Lastly, let them easily describe any specific dialect by creating a profile for it that selects specific variants for each feature. This will often have to be expressed in stochastic terms, i.e. "half the time it uses this form, half the time this form".
5. This means we have to think about what it means to be able to "easily describe" things.
    1. Help creators out by making sure that if they say, for example, that the underlying sound "X" can turn into either "x", "y", or "z" in different dialects, then when they go to define the variants a specific dialect uses they can only pick "X -> x", "X -> y", or "X -> z" (or some combination). They should never be allowed to accidentally write something like "X -> f" when they didn't even define f as an option in the first place. (This is what I was using TypeScript's actually-super-powerful type system for.)

Even though #1 indicates that the overarching goal of this project is supposed to be "be a Sprachatlas", the rest of the list actually points to a new, emergent goal: be a **sound-change applier** (SPA). SPAs are a tool I think most used in the conlang community to simulate the historical development of a language. The only differences on our part are (1) that we're not making a real historical reconstruction and (2) that we're applying more than just **sound** changes, as mentioned in the list above. We need to generalize the idea of applying rules to allow us to transform basically anything into anything else; for example, in order to accurately represent how verb conjugation varies throughout Lebanon, we need to have our tool be able to apply rules to abstract object like "verb with this root conjugated for this person/gender/number and this TAM" to turn it into a real word.

To further explain, I never saw a need to do anything fancier for this than providing a way to express simple, [SPE](https://en.wikipedia.org/wiki/Sound_Pattern_of_English)-type `A > B / environment` rules (not in that actual notation, just that type of "this becomes this when etc" rule). You just need to devise a "pretend-reconstruction" of an ancestor for the dialects you want to cover, then make sure that pretend-reconstruction is representable in code and that you can write rules that generate different dialects' forms from it. Sometimes when I would summarize this project people would wonder about its relationship to modern NLP and machine learning, and while useful ML tools could be built on top of this (e.g. a model trained to go in reverse and decide on a list of dialect features from an unannotated text), the base tool itself isn't a useful outlet for machine learning and can be conceived of in basically Linguistics 101 terms. That doesn't mean that this is an outdated/outmoded project that could be better if it used ML somehow, and it also doesn't mean we're avoiding using ML on purpose in order to prove a point. It's just that this specific project is a different type of thing than what you would need to use machine learning for.

I worked on this project on and off from my freshman year of college until just after my senior year and graduation. I was working solo and I didn't have the correct scope in mind at first (back when I was a freshman my initial idea for this tool was to demonstrate a Lebanese orthography I was naively trying to one-up Said Akl with), so it took me a long time and a lot of rethinking to finally hit on (1) the correct ultimate goal ("be a Sprachatlas") and (2) some semblance of a correct design (which I didn't figure out until my final work stint just after graduating). If this work is ever to be revived it'll have to be from scratch based on the goals I outlined in the list here above, but the redesign can still be informed a little bit by the present state of the codebase as well as the [issues](/issues) I filed back when I had just finished (for what I didn't know would be the last time, oh well). Two reasons the redesign will have to be from scratch are:

1. I didn't do enough to document the final resting state of the codebase and the bugs I had run into. Later in this README I make reference to "two" devastating bugs, but I don't remember what they are anymore other than that one was a performance issue. (It had to do with some type-system thing where I construct a tree from the bottom up (I don't remember what the tree is of anymore) and then transform it into some other format all at once instead of transforming individual subtrees as I create them, and my suspicion was that this was the cause of a major slowdown.)
2. The project relies on a lot of **really** convoluted TypeScript hacks. I have no idea how I managed to get something that actually worked (despite the two bugs, whatever they were, plus the other mentions of weird stuff scattered throughout the comments in the codebase) out of all this insanity. Unfortunately, a lot of what I relied on must have been undefined behavior, because the project no longer typechecks successfully with newer versions of the TSC compiler than 5.3.3.

I never got to the point where I made a usable frontend i.e. the map and everything else. The part I left off at was just after I had finished all of the type-system code that makes the project usable for a "creator" (meaning it's at the point where they can rely on the typechecker to make sure they're not making mistakes while documenting a language/dialect) and had started to implement and troubleshoot the runtime code that generates the actual surface forms using the data the creator describes. Because of that, this codebase actually contains an almost-complete phonological and morphological description of my own dialect, and it also contains the seeds of a Lebanese verb conjugator (in that all the data it needs is here) that I think would be more complete than anything else that's out there right now, but it doesn't have the runtime code that actually uses that data correctly.
