...
const grammar: Grammar = {
  ...
  ParserRules: [
    ...
    {name: `idafe`,
      symbols: [{literal: `(idafe`}, `idafe$ebnf$1`, `__`, `idafe$subexpression$1`, `__`, `idafe$subexpression$2`, {literal: `)`}],
      postprocess:
    ...
    {name: `number`,
      symbols: [{literal: `(`}, `number$ebnf$1`, ({type: `number`}), `number$subexpression$1`, {literal: `)`}],
      postprocess:
    ...
    {name: `number`,
      symbols: [{literal: `(`}, `number$ebnf$2`, ({type: `genderedNumber`}), ({type: `numberGender`}), `number$subexpression$2`, {literal: `)`}],
      postprocess:
    ...
    {name: `af3al`,
      symbols: [{literal: `(af3al`}, `af3al$ebnf$1`, `af3al$ebnf$2`, `__`, `root`, `af3al$ebnf$3`, {literal: `)`}],
      postprocess:
    ...
    {name: `tif3il`,
      symbols: [{literal: `(tif3il`}, `tif3il$ebnf$1`, `tif3il$ebnf$2`, `__`, `root`, `tif3il$ebnf$3`, {literal: `)`}],
      postprocess:
        ([, suffix, ctx,, root, augmentation]) => init(
          type.tif3il,
          {},
    ...
    {name: `pp`,
      symbols: [{literal: `(pp`}, `pp$ebnf$1`, `__`, `suffixed_wazn`, `__`, `voice`, `__`, `pronoun`, `__`, `root`, `pp$ebnf$2`, {literal: `)`}],
      postprocess:
    ...
    {name: `verb`,
      symbols: [{literal: `(verb`}, `verb$ebnf$1`, `__`, `wazn`, `__`, `tam`, `__`, `pronoun`, `__`, `root`, `verb$ebnf$2`, {literal: `)`}],
      postprocess:
    ...
    {name: `word`,
      symbols: [`stem`, `word$ebnf$1`, `word$ebnf$2`],
      postprocess:
    ...
    {name: `suffix$macrocall$1`,
      symbols: [`suffix$macrocall$2`, `suffix$macrocall$1$ebnf$1`],
      postprocess:
        ([suffixChain, nesteds]) =>
    ...
    {name: `suffix$macrocall$3`,
      symbols: [`suffix$macrocall$4`, `suffix$macrocall$3$ebnf$1`],
      postprocess:
        ([suffixChain, nesteds]) =>
          // XXX: the suffixChain.length stuff is bad hardcoding
          // (basically works bc the only options are C%, CG, =%, =G, and % or G on its own, so do the math)
    ...
    {name: `suffix_not_iyy$macrocall$1`,
      symbols: [`suffix_not_iyy$macrocall$2`, `suffix_not_iyy$macrocall$1$ebnf$1`],
      postprocess:
        ([suffixChain, nesteds]) =>
          // XXX: the suffixChain.length stuff is bad hardcoding
          // (basically works bc the only options are C%, CG, =%, =G, and % or G on its own, so do the math)
    ...
    {name: `suffix_not_iyy$macrocall$3`,
      symbols: [`suffix_not_iyy$macrocall$4`, `suffix_not_iyy$macrocall$3$ebnf$1`],
      postprocess:
        ([suffixChain, nesteds]) =>
          // XXX: the suffixChain.length stuff is bad hardcoding
          // (basically works bc the only options are C%, CG, =%, =G, and % or G on its own, so do the math)
    ...
    {name: `suffix_not_iyy$macrocall$5`,
      symbols: [`suffix_not_iyy$macrocall$6`, `suffix_not_iyy$macrocall$5$ebnf$1`],
      postprocess:
        ([suffixChain, nesteds]) =>
          // XXX: the suffixChain.length stuff is bad hardcoding
          // (basically works bc the only options are C%, CG, =%, =G, and % or G on its own, so do the math)
    ...
    {name: `suffix_not_iyy$macrocall$7`,
      symbols: [`suffix_not_iyy$macrocall$8`, `suffix_not_iyy$macrocall$7$ebnf$1`],
      postprocess:
        ([suffixChain, nesteds]) =>
          // XXX: the suffixChain.length stuff is bad hardcoding
          // (basically works bc the only options are C%, CG, =%, =G, and % or G on its own, so do the math)
    ...
    {name: `monosyllable`,
      symbols: [`monosyllable$macrocall$1`],
      postprocess: ([syllable]) => ({
        stressedOn: -1,
    ...
    {name: `final_superheavy_syllable`,
      symbols: [`FEM`, `DUAL`],
      postprocess:
        ([a, b]) => _.obj(
          type.syllable,
    ...
    {name: `weak_consonant`,
      symbols: [({type: `openWeakConsonant`}), `strong_consonant`, ({type: `closeWeakConsonant`})],
      postprocess:
